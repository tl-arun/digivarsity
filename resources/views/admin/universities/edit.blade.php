                ...
                <form action="{{ route('admin.universities.update', $university) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description', $university->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mt-1 flex items-center">
                            <div id="preview" class="{{ $university->logo ? '' : 'hidden' }} mr-4">
                                <img id="preview-image" src="{{ $university->logo ? asset('storage/' . $university->logo) : '' }}" alt="Preview" class="h-24 w-24 object-contain border border-gray-300 rounded-lg">
                            </div>
                            <label for="logo" class="cursor-pointer bg-white px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                                <i class="fas fa-upload mr-2"></i>{{ $university->logo ? 'Change' : 'Choose' }} File
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $university->is_active) ? 'checked' : '' }}
